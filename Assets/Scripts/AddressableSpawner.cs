...
public class AddressableSpawner : MonoBehaviour
{
   [SerializeField] private List<AssetReference> _particleReferences;
    ...
    private readonly Dictionary<AssetReference, List<GameObject>> _spawnedObgs = 
    ...
    public void Spawn(int index)
    {
        ...
        if (_asyncOperationHandles.ContainsKey(assetReference))
        {
            if (_asyncOperationHandles[assetReference].IsDone)
                AssetFromLoadedReference(assetReference, GetRandomPosition());
            else
        ...
        op.Completed += (operation) =>
        {
            ...
            if (_queuedSpawnRequests.ContainsKey(assetReference))
            {
                while (_queuedSpawnRequests[assetReference]?.Any() == true)
                {
                    ...
                    AssetFromLoadedReference(assetReference, position);
                }
            }
        };
    }
    ...
    private void AssetFromLoadedReference(AssetReference assetReference, Vector3 position)
    {
        assetReference.InstantiateAsync(position, Quaternion.identity).Completed += (asyncOperationHandle) =>
        {
            if (_spawnedObgs.ContainsKey(assetReference) == false)
            {
                _spawnedObgs[assetReference] = new List<GameObject>();
            }
            
            _spawnedObgs[assetReference].Add(asyncOperationHandle.Result);
    ...
    private void Remove(AssetReference assetReference, NotifyOnDestroy obj)
    {
        ...
        _spawnedObgs[assetReference].Remove(obj.gameObject);
        if (_spawnedObgs[assetReference].Count == 0)
        {
